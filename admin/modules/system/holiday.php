...
if (isset($_POST['saveData']) and $can_read and $can_write) {
    ...
    if (empty($holDesc)) {
        ...
        if (!preg_match('@^[0-9]{4}-[0-9]{2}-[0-9]{2}$@', $data['holiday_date'])) {
            utility::jsAlert(('Holiday Date Start must have the format YYYY-MM-DD!'));
            exit();
        }
...
if (isset($_GET['mode'])) {
    ...
    $table->appendTableRow(array(
        '<input type="checkbox" name="dayname[]" value="mon" ' . isChecked('mon') . ' /> ' . __('Monday'),
    ...
    $table->appendTableRow(array(
        '<input type="checkbox" name="dayname[]" value="thu" ' . isChecked('thu') . ' /> ' . __('Thursday'),
