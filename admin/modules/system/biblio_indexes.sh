...
if ($argv[1] == 'empty') {
	...
	if ($empty) {
		$message = 'Tabel indeks terpotong!';
	} else {
		$message = 'Tabel indeks GAGAL untuk terpotong, mungkin karena kesalahan kueri basis data!';
	}
	echo $message."\n";
	exit(0);
}
...
if ($argv[1] == 'reindex') {
	...
	$rec_bib_d = $rec_bib_q->fetch_row();
	if ($rec_bib_d[0] > 0) {
		$message = 'Harap kosongkan Indeks terlebih dahulu sebelum membuat kembali Indeks';
		echo $message."\n";
        exit(6);
	} else {
		$indexer = new biblio_indexer($dbs, $is_verbose);
