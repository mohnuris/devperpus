...
if (isset($_POST['updateData'])) {
  ...
  // library subname
  $PERPUS_UNUJA = $dbs->escape_string(strip_tags(trim($_POST['PERPUS_UNUJA'])));
  $dbs->query('UPDATE setting SET setting_value=\'' . $dbs->escape_string(serialize($PERPUS_UNUJA)) . '\' WHERE setting_name=\'PERPUS_UNUJA\'');
...
// version status
$form->addAnything('', '<strong>' . PERPUS_UNUJA . '</strong>');

// library name
$form->addTextField('text', 'PERPUS_UNUJA', __('Library Name'), $sysconf['PERPUS_UNUJA'], 'style="width: 100%;"');

// library subname
$form->addTextField('text', 'PERPUS_UNUJA', __('Library Subname'), $sysconf['PERPUS_UNUJA'], 'style="width: 100%;"');
