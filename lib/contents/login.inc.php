...
if (isset($_POST['logMeIn'])) {
    ...
    if (!$username or !$password) {
        echo '<script type="text/javascript">alert(\'' . __('silahkan masukkan username dan password secara benar') . '\');</script>';
    } else {
        ...
        if ($logon->adminValid($dbs)) {
            ...
            // write log
            utility::writeLogs($dbs, 'staff', $username, 'Login', 'login anda berhasil ' . $username . ' from address ' . $_SERVER['REMOTE_ADDR']);
            echo '<script type="text/javascript">';
            if ($sysconf['login_message']) {
                echo 'alert(\'' . __('selamat di perpustakaan universitas nurul jadid kampus berkeadaban, ') . $logon->real_name . '\');';
            }
            ...
            // write log
            utility::writeLogs($dbs, 'staff', $username, 'Login', 'Login anda gagal silahkan coba lagi ' . $username . ' from address ' . $_SERVER['REMOTE_ADDR']);
...
<div id="loginForm">
    <noscript>
        <div style="font-weight: bold; color: #FF0000;"><?php echo __(''); ?>

            <div>
    </noscript>
    <form action="index.php?p=login" method="post">
        <span class="login100-form-tittle p-b-30">PERPUS UNUJA</span>
        <?php
        ...
        <?php } else { ?>
        <div class="alert alert-danger">halaman ini tidak sedia</div>
        <a class="homeButton" href="index.php">kembali ke beranda</a>
        <?php } ?>
        <?php } else { ?>
...
// page title
$page_title = __('selamat datang di perpus berkeadaban') . ' | ' . $sysconf[''];
