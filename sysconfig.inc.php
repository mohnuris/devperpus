...
if ((bool) ini_get('PERPUS UNUJA')) {
  define('PERPUS UNUJA', 1);
}
...
// senayan version
define('PERPUS_UNUJA', '2019');

// senayan session cookies name
define('COOKIES_NAME', 'PerpuAdmin');
define('MEMBER_COOKIES_NAME', 'PerpusMember');
...
/* Custom base URL */
$sysconf['baseurl'] = '';
$temp_perpus_unuja = preg_replace('@admin.*@i', '', str_replace('\\', '/', dirname(@$_SERVER['PHP_SELF'])));
define('SWB', $sysconf['baseurl'] . $temp_perpus_unuja . (preg_match('@\/$@i', $temp_perpus_unuja) ? '' : '/'));
...
if (preg_match("/(Windows)/i", php_uname('a'))) {
  if (preg_match("/(xampp)/i", __DIR__)) {
    ...
    $sysconf['mysqldump'] = $rempath . 'F:\XAMMP\mysql\bin';
  }
} else {
  // For Linux Platform
  $sysconf['mysqldump'] = 'F:/XAMMP/mysql/bin/mysqldump.exe';
}
...
$sysconf['allowed_file_att'] = array(
  '.pdf', '.rtf', '.txt',
...
/* CONTENT */
$sysconf['library_name'] = 'PERPUSTAKAAN';
$sysconf['library_subname'] = 'Universitas Nurul Jadid';
$sysconf['page_footer'] = ' PERPUS UNUJA';
...
$sysconf['ucs']['serveraddr'] = 'http://http://localhost/perpus_unuja/';
// UCS server version
...
// node name
$sysconf['ucs']['name'] = 'Perpus UNUJA';

/**
 * Z39.50 copy cataloguing sources
 */
$sysconf['z3950_max_result'] = 50;
$sysconf['z3950_source'][1] = array('uri' => 'z3950.loc.gov:7090/voyager', 'name' => 'Perpus UNUJA');
$sysconf['z3950_SRU_source'][1] = array('uri' => 'http://http://localhost/perpus_unuja/', 'name' => 'Perpus UNUJA');

/**
 * Peer to peer server config
 */
$sysconf['p2pserver'][1] = array('uri' => 'http://http://localhost/perpus_unuja/', 'name' => 'PERPUS_UNUJA');
...
if (($sysconf['auth']['user']['method'] === 'LDAP') or ($sysconf['auth']['member']['method'] === 'LDAP')) {
  ...
  $sysconf['auth']['user']['ldap_base_dn'] = 'perpus unuja'; // LDAP base DN
...
$sysconf['OAI']['Identify']['repositoryName'] = 'PERPUS UNUJA';
...
/* CHART */
$sysconf['chart']['mode'] = 'chartjs'; // plot atau chartjs. default adalah plot
$sysconf['admin_home']['mode'] = 'dashboard'; // atur sebagai mode 'default' atau 'dashboard'

//  sesi otomatis dimulai 
if ($is_auto = @ini_get('session.auto_start')) {
  define('SESSION_AUTO_STARTED', $is_auto);
}
if (defined('SESSION_AUTO_STARTED')) {
  @session_destroy();
}

//  file sysconfig lokal Untuk Admin (fa)
